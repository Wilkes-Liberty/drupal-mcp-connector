...
> With the **MCP Sentinel** module installed, bind this role to an `mcp_policy_profile` and issue the connector an OAuth token (Consumer with `mcp:read`/`mcp:write` scopes). Governance and audit then key off the authenticated role and scopes server-side. See the [MCP Sentinel project page](https://www.drupal.org/project/mcp_sentinel).
