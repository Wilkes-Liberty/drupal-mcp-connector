/**
 * One Sentinel HTTP client and one fail-closed absence policy
 * (d.o #3621022 / GitHub #176).
 *
 * Path builders, If-Match, lang/state headers, missing-endpoint classifiers,
 * validated inventory, and working-revision rewrites live here. Tools call
 * these functions; they must not sniff GraphQL `rawQuery` or copy the
 * missing-endpoint English sentence.
 */
 ...
 * Otherwise requires JSON:API `resourcePath` plus `rawQuery`. GraphQL
 * `rawQuery({ query })` does not count.
 * @param {object} [backend]
 * @returns {boolean}
 */
export function supportsSentinelDraft(backend) {
  ...
  if (typeof backend.capabilities === "function") {
    ...
    if (caps && typeof caps === "object" && Object.hasOwn(caps, "sentinelDraft")) {
      return caps.sentinelDraft === true;
    }
  }
  return typeof backend.rawQuery === "function" && typeof backend.resourcePath === "function";
}

/**
 * Whether an error is Sentinel's missing governed draft endpoint (404/405).
 * @param {unknown} error
 * @returns {boolean}
 */
export function isMissingDraftEndpoint(error) {
  return MISSING_DRAFT_RE.test(String(error?.message ?? ""));
}

/**
 * Whether an error is Sentinel's missing governed draft-translation endpoint
 * (404/405). Permission, transport, and malformed inventory are not absence.
 * @param {unknown} error
 * @returns {boolean}
 */
export function isMissingTranslationEndpoint(error) {
  return MISSING_TRANSLATION_RE.test(String(error?.message ?? ""));
}
...
/**
 * One Sentinel HTTP request. Node/media vs paragraph differ only in the
 * If-Match value and optional `X-MCP-Draft-State` — callers build those.
 * @param {object} backend
 * @param {object} spec
 * @param {string} spec.entityType
 * @param {string} spec.bundle
 * @param {string} spec.id
 * @param {string} spec.suffix Path after the entity UUID (`mcp-draft`,
 *   `mcp-draft/translations`, `mcp-translations`).
 * @param {object} [spec.options] Passed to JSON:API `rawQuery`.
 * @param {string} spec.missingMessage Absence rewrite for Drupal 404/405.
 * @returns {Promise<*>}
 */
async function sentinelDraftRequest(backend, {
  entityType, bundle, id, suffix, options, missingMessage,
}) {
  const base = draftResource(backend, entityType, bundle, id);
  try {
    return await backend.rawQuery({
      path: `${base}/${suffix}`,
      options,
    });
  } catch (error) {
    throw missingEndpointError(error, missingMessage);
  }
}
...
export async function createTranslationDraft(backend, input, preflight = false) {
  ...
  try {
    result = await sentinelDraftRequest(backend, {
      entityType, bundle, id, suffix: "mcp-draft/translations",
    ...
    throw rewriteTranslationWorkingRevisionError(error);
  }
  if (preflight) {
 ...
 *
 * Missing 404/405 inventory is absence and may fall back to the working-copy
...
export async function resolveNodeTranslationPair(backend, { entityType, bundle, id, existing }) {
  ...
  let meta;
  try {
    meta = await readTranslationInventory(backend, { entityType, bundle, id });
  } catch (error) {
...
export async function readTranslationInventory(backend, { entityType, bundle, id, revisionId }) {
  ...
  const result = await sentinelDraftRequest(backend, {
    entityType, bundle, id, suffix: "mcp-translations",
    missingMessage: MISSING_TRANSLATION_ENDPOINT,
...
export async function readNodeDraftInventory(backend, ref) {
  if ((ref.entityType !== "node" && ref.entityType !== "media")
    || !supportsSentinelDraft(backend)) return null;
  let inventory;
  try {
    inventory = await readTranslationInventory(backend, ref);
  } catch (error) {
    if (isMissingTranslationEndpoint(error)) return null;
    throw error;
  }
...
export async function readDraftTranslation(backend, input) {
  ...
  const { live, working } = requireWorkingPair(draftRevision);
  const result = await sentinelDraftRequest(backend, {
    entityType, bundle, id, suffix: "mcp-draft",
    missingMessage: MISSING_TRANSLATION_ENDPOINT,
    options: {
      method: "GET",
      headers: {
